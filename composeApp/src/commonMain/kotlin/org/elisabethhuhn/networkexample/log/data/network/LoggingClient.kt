...
import org.elisabethhuhn.networkexample.LOGGER_HOST
...
class LoggingClient(
    ...
    suspend fun logRemote(logEntriesList: List<LogEntry>): Result<String, NetworkError> {
        val response = try {

            httpClient.post(
                urlString =  LOGGER_HOST
            ) {
                contentType(ContentType.Application.Json)
                setBody(logEntriesList)
            }
...
//            return Result.Error(NetworkError.CONNECTION_FAILED)
        ...
        return when(returnCode) {
            in 200..299 -> {
                Result.Success("Message Logged Successfully!!!!!!")
            }
